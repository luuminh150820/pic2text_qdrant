...
def process_images_parallel(image_paths, max_workers=1):
    """Process images in parallel for better performance"""
    results = {}
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_image_to_text, path): path for path in image_paths}
        for future in tqdm(futures, desc="Processing Images"): # Added tqdm here
            path = futures[future]
            try:
                text = future.result()
                if text:
...
def main():
    ...
    total_chunks = 0
    chunk_id_counter = 0
    for image_path, text in tqdm(extracted_texts.items(), desc="Processing Text Chunks"): #Added Tqdm here
    ...
    print(f"Stored {total_chunks} chunks in Qdrant at {QDRANT_LOCAL_PATH}")
